...
class EmployeDAO
{
    ...
    public function addEmploye($e){
        $reqPre = $this->pdo->prepare("INSERT INTO employe (CIN , nom, prenom, dateNaissance, tel, email , sexe, adresse, nbrEnfant, passwd, photo, role, dateRecrutement, codeF, codeC, num)
    ...
    public function getEmployeByCIN($cin){
        $reqPre = $this->pdo->prepare("SELECT * FROM employe WHERE CIN = :cin");
        $reqPre->setFetchMode(PDO::FETCH_OBJ);
        $reqPre->execute(array(":cin"=>$cin));
    ...
    public function getEmployeByEmail($email){
        $reqPre = $this->pdo->prepare("SELECT * FROM employe WHERE email = :email");
        $reqPre->setFetchMode(PDO::FETCH_OBJ);
        $reqPre->execute(array(":email"=>$email));
        $listF = $reqPre->fetchAll();
        return $listF[0];
    }
}